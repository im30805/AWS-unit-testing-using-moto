...
class TestAdminDeleteSupplier:
    ...
    def test_lambda_function(self, lambda_context):
        lambda_function = importlib.import_module("lambda.functions.ECOM-AdminDeleteSupplier.lambda_function")
    ...
    def test_lambda_function_fail_delete_exception(self, lambda_context): # this test will fail, exception is not raised since it is actually already handled
        import uuid
        ...
        with pytest.raises(Exception):
            lambda_function.lambda_handler(example_event, lambda_context)  # should raise the exception 'Failed to delete Supplier'
    ...
    def test_lambda_function_empty_event(self, lambda_context):  # this test will fail, exception not raised, will return the standard error msg
        ...
        with pytest.raises(Exception):
            lambda_function.lambda_handler(example_event, lambda_context)  # should raise the exception 'Invalid arguments'